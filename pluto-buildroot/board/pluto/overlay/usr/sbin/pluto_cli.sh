...
case $command in
	...
	TXGAIN ) echo "    **** set TX attenuation  $arg1"
		echo
		/usr/bin/iio_attr -q -c -o ad9361-phy voltage0 hardwaregain $((arg1)) 1>/dev/null
		get_status;		
		;;
	...
	RXGAIN ) echo "    **** set RX gain  $arg1"
		echo
		/usr/bin/iio_attr -q -c -i ad9361-phy voltage0 gain_control_mode manual 1>/dev/null
		/usr/bin/iio_attr -q -c -i ad9361-phy voltage0 hardwaregain $((arg1)) 1>/dev/null
		get_status;		
		;;
	SR) echo "    **** set TX/RX Samplerate  $arg1"
		/usr/bin/iio_attr -q -c ad9361-phy voltage0 sampling_frequency $((arg1)) 1>/dev/null
		get_status;		
		;;
