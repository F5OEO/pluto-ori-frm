...
-- Blocks
local source = radio.SoapySDRSource("driver=plutosdr",frequency + tune_offset, 1102500, {gain = 60})
...
-- local sink = radio.NetworkServerSink('json', 'tcp', '0.0.0.0:1111')
-- local sink = radio.RawFileSink('/tmp/toto.txt')
-- local sink = radio.NetworkClientSink('json', 'tcp', '127.0.0.1:1111')
-- Plotting sinks
...
-- top:connect(sampler, bit_demod, bit_slicer, bit_decoder, framer, sink)
