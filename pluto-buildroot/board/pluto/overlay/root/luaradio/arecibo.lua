...
local source = radio.RawFileSource('./arecibo.txt', radio.types.Bit,50)
-- local source = radio.UniformRandomSource(radio.types.Bit, 1200)
local bfsk = BFSKModulator(5e3 , 48e3)
local interpolator = radio.InterpolatorBlock(2)
local sink = radio.IQFileSink('/tmp/iq.test', 'f32le')
...
local plutoiq = radio.CompositeBlock()
      plutoiq:connect(source,bfsk,sink)
